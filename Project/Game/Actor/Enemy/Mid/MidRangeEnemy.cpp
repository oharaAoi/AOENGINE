...
namespace {
constexpr int kEnemyRenderQueue = 100;
}
...
void MidRangeEnemy::Init() {
	...
	// ↓ objectをシーンに追加する
	// ----------------------
	object_ = AOENGINE::SceneRenderer::GetInstance()->AddObject<AOENGINE::BaseGameObject>("MidRangeEnemy", "Object_PBR.json", kEnemyRenderQueue);
	InitCommon();

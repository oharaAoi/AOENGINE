...
void MidRangeEnemy::Init() {
	...
	// ↓ objectをシーンに追加する
	// ----------------------
	object_ = AOENGINE::SceneRenderer::GetInstance()->AddObject<AOENGINE::BaseGameObject>("MidRangeEnemy", "Object_PBR.json", 100);
	InitCommon();
	...
	// ↓ 武器を生成
	// ----------------------
	weapon_ = std::make_unique<LauncherGun>();
	weapon_->Init();
	...
	behaviorTree_->Init();
	behaviorTree_->SetName("MidRangeEnemyTree.aitree");
	...
	behaviorTree_->CreateTree("./Project/Packages/Game/Assets/GameData/BehaviorTree/", "MidRangeEnemyTree.aitree");

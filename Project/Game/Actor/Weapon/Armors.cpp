...
namespace {
constexpr int kArmorRenderQueue = 100;
}
...
void Armors::Init(const std::string& ownerName) {
	SetName("Armors");
	outArmor_ = AOENGINE::SceneRenderer::GetInstance()->AddObject<PulseArmor>("BossPulseArmor", "Object_Dissolve.json", kArmorRenderQueue);
	inArmor_ = AOENGINE::SceneRenderer::GetInstance()->AddObject<PulseArmor>("BossPulseArmor", "Object_Dissolve.json", kArmorRenderQueue);

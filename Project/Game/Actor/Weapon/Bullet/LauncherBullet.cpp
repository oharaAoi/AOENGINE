...
namespace {
constexpr float kBulletDeadDistance = 200.0f;
}
...
void LauncherBullet::Update() {
	...
	if (std::abs(pos.x) >= kBulletDeadDistance) {
		isAlive_ = false;
	}

	if (std::abs(pos.y) >= kBulletDeadDistance) {
		isAlive_ = false;
	}

	if (std::abs(pos.z) >= kBulletDeadDistance) {
		isAlive_ = false;
	}

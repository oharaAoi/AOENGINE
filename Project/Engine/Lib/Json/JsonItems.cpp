...
const std::string JsonItems::sDirectoryPath_ = kAssetPath + "/Game/GameData/JsonItems/";
...
void JsonItems::LoadAllFile() {
	fs::path objectDir = fs::path(sDirectoryPath_) / "Object";

	std::string currentPath = fs::current_path().string();

	std::cout << "current_path: " << currentPath << std::endl;
	std::cout << "objectDir: " << fs::absolute(objectDir) << std::endl;

	jsonMap_.clear();
	// objectの保存項目の読み込み
	if (std::filesystem::exists(sDirectoryPath_ + "Object")) {
	...
	// effect読み込み
	if (std::filesystem::exists(sDirectoryPath_ + "Effect")) {

...
ComPtr<IDxcBlob> DirectXCompiler::VsShaderCompile(const std::string& _shader) {
	...
	std::string path = kAssetPath + _shader;
	result = CompilerShader(ConvertWString(path), L"main", L"vs_6_0", dxcUtils_.Get(), dxcCompiler_.Get(), includeHandler_.Get());
...
ComPtr<IDxcBlob> DirectXCompiler::PsShaderCompile(const std::string& _shader) {
	ComPtr<IDxcBlob> result{};
	std::string path = kAssetPath + _shader;
	result = CompilerShader(ConvertWString(path), L"main", L"ps_6_0", dxcUtils_.Get(), dxcCompiler_.Get(), includeHandler_.Get());
	assert(result != nullptr);
...
ComPtr<IDxcBlob> DirectXCompiler::CsShaderCompile(const std::string& _shader) {
	ComPtr<IDxcBlob> result{};
	std::string path = kAssetPath + _shader;
	result = CompilerShader(ConvertWString(path), L"CSmain", L"cs_6_0", dxcUtils_.Get(), dxcCompiler_.Get(), includeHandler_.Get());
	assert(result != nullptr);

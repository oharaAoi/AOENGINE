...
void ProcessedScenePipelines::Init(ID3D12Device* device, DirectXCompiler* dxCompiler, Shader* shaders) {
	...
	pipelineMap_[ProcessedScenePSO::RadialBlur]->Initialize(device, dxCompiler, shaders->GetShaderData(Shader::RadialBlur), RootSignatureType::RadialBlur, inputLayout_.CreateRenderTexture(), Blend::ModeNone, true, false);
	pipelineMap_[ProcessedScenePSO::GlitchNoise]->Initialize(device, dxCompiler, shaders->GetShaderData(Shader::GlitchNoise), RootSignatureType::RadialBlur, inputLayout_.CreateRenderTexture(), Blend::ModeNone, true, false);
}

...
void SphereGeometry::Debug_Gui() {
	ImGui::DragFloat2("size", &size_.x, 0.1f);

	const float kLonEvery = float(kPI) * 2.0f / float(division_);// fai
	const float kLatEvery = float(kPI) / float(division_); // theta
	...
	for (uint32_t latIndex = 0; latIndex < division_; ++latIndex) {
		...
		for (uint32_t lonIndex = 0; lonIndex < division_; ++lonIndex) {
			start = (latIndex * division_ + lonIndex) * 4;
			float lon = lonIndex * kLonEvery; // fai
			// 頂点データを入力
			// a 左下
			vertexData_[start].pos.x = std::cos(lat) * std::cos(lon);
			...
			vertexData_[start].texcoord.x = float(lonIndex) / float(division_);
			vertexData_[start].texcoord.y = 1.0f - float(latIndex) / float(division_);
			vertexData_[start].normal = { vertexData_[start].pos.x, vertexData_[start].pos.y, vertexData_[start].pos.z };
			...
			vertexData_[start + 1].texcoord.x = float(lonIndex) / float(division_);
			vertexData_[start + 1].texcoord.y = 1.0f - float(latIndex + 1) / float(division_);
			vertexData_[start + 1].normal = { vertexData_[start + 1].pos.x, vertexData_[start + 1].pos.y, vertexData_[start + 1].pos.z };
			...
			vertexData_[start + 2].texcoord.x = float(lonIndex + 1) / float(division_);
			vertexData_[start + 2].texcoord.y = 1.0f - float(latIndex) / float(division_);
			vertexData_[start + 2].normal = { vertexData_[start + 2].pos.x, vertexData_[start + 2].pos.y, vertexData_[start + 2].pos.z };
			...
			vertexData_[start + 3].texcoord.x = float(lonIndex + 1) / float(division_);
			vertexData_[start + 3].texcoord.y = 1.0f - float(latIndex + 1) / float(division_);
			vertexData_[start + 3].normal = { vertexData_[start + 3].pos.x, vertexData_[start + 3].pos.y, vertexData_[start + 3].pos.z };

			//
			startIndex = (latIndex * division_ + lonIndex) * 6;
	...
	for (uint32_t oi = 0; oi < vertexData_.size(); ++oi) {
		vertexData_[oi].pos.x *= size_.x;
		vertexData_[oi].pos.y *= size_.y;
		vertexData_[oi].pos.z *= size_.x;
	}
}
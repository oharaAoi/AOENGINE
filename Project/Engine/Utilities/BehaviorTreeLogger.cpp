...
namespace {
constexpr size_t kMaxBehaviorTreeLogFiles = 20;
}
...
void BehaviorTreeLogger::Init(const std::string& _fileName) {
	try {
		std::filesystem::create_directories("./Logs/BehaviorTree");
	}
	...
	// ファイル数が多くなりすぎないように
	DeleteOldLogFile(kMaxBehaviorTreeLogFiles);
	...
	// 時刻を使ってファイル名を決定
	filePath_ = std::string("./Logs/BehaviorTree/") + path.stem().string() + "_" + dateString + ".log";
	// ファイルを作って書き込み準備
...
void BehaviorTreeLogger::DeleteOldLogFile(size_t max) {
	const std::filesystem::path logDir = "./Logs/BehaviorTree/";

#include "AssetsWindowSerializer.h"
#include <fstream>
#include "Engine/Lib/Path.h"

void AOENGINE::AssetsWindowSerializer::Save(const std::unordered_map<std::string, bool>& stateMap, const std::string& currentPath) {
	const std::string filePath = kAssetPath + "/Property/AssetsWindow.json";
	nlohmann::json data;
...
void AOENGINE::AssetsWindowSerializer::Load(std::unordered_map<std::string, bool>& map, std::string& currentPath) {
	const std::string filePath = kAssetPath + "/Property/AssetsWindow.json";
	// 読み込み用ファイルストリーム
	std::ifstream ifs;
	// ファイルを読み込みように開く

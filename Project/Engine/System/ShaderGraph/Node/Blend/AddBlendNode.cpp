...
void AddBlendNode::customUpdate() {
	// 入力からの受け取り
	...
	if (resourceA_ && resourceB_) {
		if (blendResource_->GetResource()) {
			...
			Pipeline* pso = Engine::SetPipelineCS("AddBlend.json");
			UINT index = 0;
...
nlohmann::json AddBlendNode::toJson() {
	nlohmann::json result;
	BaseInfoToJson(result);
	return result;
}

void AddBlendNode::fromJson(const nlohmann::json& _json) {
	BaseInfoFromJson(_json);
}

void AddBlendNode::CreateBlendResource() {
	if (resourceA_) {
		if (!blendResource_->GetResource()) {
